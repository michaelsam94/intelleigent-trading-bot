...
| **BTCUSDC + ETHUSDC** | Run **two processes** (one config per pair): `config-1min-realtime.jsonc` (BTCUSDC) and `config-1min-realtime-ethusdc.jsonc` (ETHUSDC). Data: `data/BTCUSDC/`, `data/ETHUSDC/`. Example: `python -m service.server -c configs/config-1min-realtime.jsonc` and same with `-c configs/config-1min-realtime-ethusdc.jsonc` (or two PM2 apps). |
